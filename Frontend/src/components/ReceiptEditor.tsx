...
export function computeReceiptTotals(draft: ReceiptDraft) {
  const subtotal = round2(draft.items.reduce((sum, item) => sum + round2((Number(item.quantity) || 0) * (Number(item.unitPrice) || 0)), 0));
...
export default function ReceiptEditor({ draft, currency, onChange, onRemove }: ReceiptEditorProps) {
  ...
  const updateItem = (index: number, changes: Partial<ReceiptItemRequest>) => patch({ items: draft.items.map((item, i) => (i === index ? { ...item, ...changes } : item)) });

  const updateCharge = (index: number, changes: Partial<ReceiptChargeRequest>) => patch({ charges: draft.charges.map((charge, i) => (i === index ? { ...charge, ...changes } : charge)) });

  const inputClass = "w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-body-md text-on-surface placeholder-on-surface-variant/40 focus:border-primary/50 focus:outline-none";
  ...
  return (
    <div className="glass-card flex flex-col gap-6 rounded-xl p-5">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={onRemove} className="font-label-md text-label-md flex items-center gap-1 text-error transition-opacity hover:opacity-80">
          <Icon name="delete" size={16} />
          Remove
        </button>
      </div>

      <input value={draft.merchant} onChange={(event) => patch({ merchant: event.target.value })} placeholder="Merchant (optional)" className={inputClass} />
      ...
      <div className="flex flex-col gap-3">
        ...
        {draft.items.map((item, index) => (
          <div key={index} className="flex flex-col gap-2 rounded-lg border border-white/5 bg-white/[0.03] p-3">
            <div className="flex items-center gap-2">
              <input value={item.name} onChange={(event) => updateItem(index, { name: event.target.value })} placeholder="Item name" className={inputClass} />
              <button
                type="button"
            ...
            <div className="flex items-center gap-2">
              <label className="flex flex-1 items-center gap-2">
                ...
                <NumberField step="0.001" min="0.001" value={item.quantity} onValueChange={(quantity) => updateItem(index, { quantity })} className={inputClass} />
              </label>
              <label className="flex flex-[2] items-center gap-2">
                ...
                <NumberField step="0.01" min="0" value={item.unitPrice} onValueChange={(unitPrice) => updateItem(index, { unitPrice })} inputMode="text" className={inputClass} />
              </label>
              <span className="font-body-md text-body-md w-24 shrink-0 text-right text-on-surface">{formatMoney(round2((Number(item.quantity) || 0) * (Number(item.unitPrice) || 0)), currency)}</span>
            </div>
          </div>
        ))}
      ...
      {/* Extra charges */}
      <div className="flex flex-col gap-3">
        <p className="font-label-md text-label-md uppercase tracking-wider text-on-surface-variant opacity-70">Taxes &amp; Charges</p>

        {draft.charges.map((charge, index) => (
          <div key={index} className="flex flex-col gap-2 rounded-lg border border-white/5 bg-white/[0.03] p-3">
            <div className="flex items-center gap-2">
              <input value={charge.label} onChange={(event) => updateCharge(index, { label: event.target.value })} placeholder="e.g. SST" className={inputClass} />
              <button
                type="button"
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex shrink-0 rounded-lg border border-white/10 bg-black/20 p-0.5">
                {(["Percentage", "Fixed"] as ChargeType[]).map((option) => (
                  <button
                    ...
                    className={cn("font-label-md text-label-md rounded-md px-3 py-1.5 transition-colors", charge.type === option ? "bg-primary/20 text-primary" : "text-on-surface-variant hover:text-on-surface")}
                  >
              ...
              <NumberField step="0.01" min="0" value={charge.value} onValueChange={(value) => updateCharge(index, { value })} className={inputClass} />

              <span className="font-body-md text-body-md w-24 shrink-0 text-right text-on-surface">{formatMoney(totals.resolved[index]?.amount ?? 0, currency)}</span>
            </div>
          </div>
        ))}
